...
def print_config(config: AquaGuardConfig):
    ...
    logger.info(f"Storage Directories:")
    ...
    logger.info(f"Actuations      : {'ENABLED (Production)' if config.hardware.enable_hardware_control else 'DISABLED (Simulated)'}")
    ...
    logger.info(f"Consent Rules   : {config.consent.regional_council} (ID: {config.consent.consent_id})")
    logger.info(f"Thresholds:")
    logger.info(f"  pH            : {config.thresholds.ph_min} - {config.thresholds.ph_max}")
